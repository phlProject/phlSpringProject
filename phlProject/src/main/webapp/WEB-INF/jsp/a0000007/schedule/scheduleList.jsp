...
<script type="text/javascript">
	...
	function controller(target) {
		...
		this.onPrevCalendar = function() {
			...
			$.post("/a0000007/schedule/scheduleListAjax.do", $("#scheduleForm").serialize(),function(data){
				var j = 0;
				cList = new Array();
	...
	function insertSchedule(){
		if(validation() && confirm("등록하시겠습니까?")){
			var url = "/a0000007/schedule/insertSchedule.do";
			$.post(url,$("#scheduleForm").serialize(),function(data){
				if(data.result == "0" ){
					alert("등록되었습니다.");
					$( "#scheduleForm").attr({action:'/a0000007/schedule/scheduleList.do' ,method:'post'}).submit();
				} else{
					alert("등록에 실패했습니다.");
					return;
				}
	...
	function updateSchedule(){
		if(validation() && confirm("수정하시겠습니까?")){
			var url = "/a0000007/schedule/updateSchedule.do";
			$.post(url,$("#scheduleForm").serialize(),function(data){
				if(data.result == "0" ){
					alert("수정되었습니다.");
					$( "#scheduleForm").attr({action:'/a0000007/schedule/scheduleList.do' ,method:'post'}).submit();
	...
	function deleteSchedule(){
		if(confirm("삭제하시겠습니까?")){
			var url = "/a0000007/schedule/deleteSchedule.do";
			$.post(url,$("#scheduleForm").serialize(),function(data){
				if(data.result == "0" ){
					alert("삭제되었습니다.");
					$( "#scheduleForm").attr({action:'/a0000007/schedule/scheduleList.do' ,method:'post'}).submit();
	...
	function popWorkDetail(scheduleSn){
		...
		var url = "/a0000007/schedule/scheduleView.do";
		$.post(url,$("#scheduleForm").serialize(),function(data){
